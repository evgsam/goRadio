...
func inputMenuForm(guiCh chan *gocui.Gui) {
	...
	*/
	...
	// new form
	signup := &signupF{
		component.NewForm(g, " IC-78 Set ", 0, 0, 20, 5), //displayAcces,
	}
	signup.AddInputField_("Freque", 0, 0, 6, 10).
		AddValidate("required input", requireValidatorF)

	// add select
	signup.AddSelect_("Mode", 18, 0, 4, 5).AddOptions("LSB", "USB", "CW", "RTTY", "AM")
	signup.AddSelect_("ATT", 29, 0, 3, 4).AddOptions("ON", "OFF")
	signup.AddSelect_("Preamp", 37, 0, 6, 6).AddOptions("P.AMP", "OFF")

	signup.AddInputField_("AF", 7, 2, 4, 5).
		AddValidate("required input", requireValidatorF)

	signup.AddInputField_("RF", 17, 2, 4, 5).
		AddValidate("required input", requireValidatorF)

	signup.AddInputField_("SQL", 27, 2, 4, 5).
		AddValidate("required input", requireValidatorF)
	// add button
	signup.AddButton("SEND Settins", signup.registF)
	signup.AddButton("Cancel", quitF)
